...
2. **Fetches listings** from Bienici, Trente Cinq Notaires, Action Logement, Giboire, Afedim and Ouest France Immo (configurable filters for Bienici; Trente Cinq Notaires: page 1, locations only; Action Logement: API, 50 km Rennes, T2/T3, max rent from Config; Giboire: Ille-et-Vilaine 35, price/surface/rooms from Config; Afedim: Ille-et-Vilaine, pièces/surface/budget from Config; Ouest France Immo: API louer Rennes 35, pièces/surface/prix from Config, tri date_desc).
...
├── workflows/
│   └── recherche_appart_rennes.json   # Main workflow (Bienici + Trente Cinq Notaires + Action Logement + Giboire + Afedim + Ouest France Immo + Sheet + Groq + Telegram)
├── .github/workflows/
