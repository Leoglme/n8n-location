...
├── workflows/
│   └── recherche_appart_rennes.json   # Main workflow (Bienici + Trente Cinq Notaires + Action Logement + Giboire + Afedim + Ouest France Immo + Foncia + La Française Immobilière + Guenno + Lamotte + Kermarrec + Nestenn + Square Habitat + Sheet + Groq + Telegram)
├── .github/workflows/
...
2. Connect that branch to **Fusionner branches 12** (or create **Fusionner branches 13** and wire merge 12 → merge 13, new source → merge 13, merge 13 → Normaliser annonces).
