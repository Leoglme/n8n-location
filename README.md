...
├── workflows/
│   └── recherche_appart_rennes.json   # Main workflow (Bienici + Trente Cinq Notaires + Action Logement + Giboire + Afedim + Ouest France Immo + Foncia + La Française Immobilière + Guenno + Lamotte + Kermarrec + Nestenn + Sheet + Groq + Telegram)
├── .github/workflows/
...
2. Connect that branch to **Fusionner branches 11** (or create **Fusionner branches 12** and wire merge 11 → merge 12, new source → merge 12, merge 12 → Normaliser annonces).
