...
├── workflows/
│   └── recherche_appart_rennes.json   # Main workflow (Bienici + Trente Cinq Notaires + Action Logement + Giboire + Afedim + Ouest France Immo + Foncia + La Française Immobilière + Guenno + Sheet + Groq + Telegram)
├── .github/workflows/
...
2. Connect that branch to **Fusionner branches 8** (or create **Fusionner branches 9** and wire merge 8 → merge 9, new source → merge 9, merge 9 → Normaliser annonces).
