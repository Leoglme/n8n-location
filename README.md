...
2. **Fetches listings** from Bienici, Trente Cinq Notaires and Action Logement (configurable filters for Bienici; Trente Cinq Notaires: page 1, locations only; Action Logement: API, 50 km Rennes, T2/T3, max rent from Config).
...
├── workflows/
│   └── recherche_appart_rennes.json   # Main workflow (Bienici + Trente Cinq Notaires + Action Logement + Sheet + Groq + Telegram)
├── .github/workflows/
...
The workflow already has a **Fusionner branches** (merge) node that combines:

- **Input 1:** Bienici (HTTP Request + transform to common format).
- **Input 2:** Trente Cinq Notaires (HTTP Request page 1 → parse HTML → same format).
- **Fusionner branches 2** merges the above with **Action Logement** (POST API → transform to same format).
