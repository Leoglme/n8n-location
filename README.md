...
├── workflows/
│   └── recherche_appart_rennes.json   # Main workflow (Bienici + Trente Cinq Notaires + Action Logement + Giboire + Afedim + Ouest France Immo + Foncia + La Française Immobilière + Guenno + Lamotte + Sheet + Groq + Telegram)
├── .github/workflows/
...
2. Connect that branch to **Fusionner branches 9** (or create **Fusionner branches 10** and wire merge 9 → merge 10, new source → merge 10, merge 10 → Normaliser annonces).
