...
**Planned:** Other agencies and listing sites will be added to the same workflow (e.g. via the existing "Other sites" placeholder branch).
...
2. **Fetches listings** from Bienici (and later other sources) using configurable filters (price, surface, rooms, areas).
...
├── workflows/
│   └── recherche_appart_rennes.json   # Main workflow (Bienici + Sheet + Groq + Telegram)
├── .github/workflows/
...
- **Input 1:** Bienici (and any other HTTP/API nodes you add).
- **Input 2:** An **Autres sites (placeholder)** no-op node.
